...
- **Mobile-First**: React Native 0.72+ via Expo SDK 49+ for cross-platform deployment
- **AI-Centric**: Google Gemini 2.5 Flash-Lite with thinking model for superior character consistency and 1.5x faster responses
...
3. **AI Service** (Gemini 2.5 Flash-Lite) - Conversation engine with adaptive thinking and context persistence
...
**Frontend**: React Native 0.72+, Expo SDK 49+, React Query, Zustand, React Hook Form, NativeBase, React Native Gifted Chat, React Navigation 6.x, RevenueCat SDK, NativeWind 4.1  
**Backend**: FastAPI (Python), Supabase, SQLAlchemy ORM, Redis + RQ, Google Gemini 2.5 Flash-Lite API  
...
**Core Framework**:
- React Native 0.72+ via Expo SDK 49+ for cross-platform development
...
**AI Integration**:
- Primary: Google Gemini 2.5 Flash-Lite with thinking model for realistic character embodiment
- Fallback: Google Gemini 2.0 Flash for reliability when primary is unavailable
- Adaptive reasoning for maintaining complete character consistency
- Context caching for 75% cost reduction and improved performance
- Custom prompt engineering with difficulty-aware thinking budgets
...
```python
# settings.py - Environment setup for Gemini
import os
import logging
...
# Custom exceptions for Gemini API
...
def handle_gemini_error(func):
    """Decorator to handle Gemini API errors with proper logging."""
    import functools
    ...
    async def wrapper(*args, **kwargs):
        try:
            ...
            logging.error(f"Gemini rate limit exceeded: {e}")
            raise GeminiRateLimitError("Rate limit exceeded. Please try again later.") from e
        except exceptions.InvalidArgument as e:
            if "blocked" in str(e).lower():
                logging.warning(f"Content blocked by safety filters: {e}")
                raise GeminiSafetyError("Content was blocked by safety filters.") from e
            raise GeminiApiError(f"Invalid request: {e}") from e
        except Exception as e:
            logging.error(f"Unexpected Gemini API error: {e}")
...
```python
# Context caching for cost optimization and performance
class GeminiContextCache:
    def __init__(self):
        self.cache_client = genai.caching
    ...
    async def create_context_cache(
        ...
        """Create a cached context for reuse across conversations."""
        ...
        cache = await self.cache_client.CachedContent.create_async(
            model='models/gemini-2.5-flash-lite',
            system_instruction=system_prompt,
            ttl=datetime.timedelta(minutes=ttl_minutes),
            display_name=f'FlirtCraft {scenario_type} context'
        )
        
        self.cached_contexts[cache_key] = cache.name
        return cache.name
    
    async def get_cached_model(self, cache_name: str) -> genai.GenerativeModel:
        """Get a model with cached context."""
...
class ConversationEngine:
    def __init__(self):
        ...
        self.primary_model = genai.GenerativeModel(
            ...
            safety_settings=self.config.default_safety_settings
        )
        self.fallback_model = genai.GenerativeModel(
            model_name=self.config.fallback_model,
            safety_settings=self.config.default_safety_settings
        )
        self.context_cache = GeminiContextCache()
    ...
    async def generate_response(
        ...
        # Build conversation with Gemini format - convert history to Gemini format
        gemini_history = self._convert_history_to_gemini(conversation_history)
        ...
        try:
            chat = self.primary_model.start_chat(history=gemini_history)
            ...
            chat = self.fallback_model.start_chat(history=gemini_history)
            response = await chat.send_message_async(
                system_prompt,
                generation_config=generation_config
            )
    ...
    def _convert_history_to_gemini(self, conversation_history: List[Dict]) -> List[Dict]:
...
"""
    ...
    # Configure Gemini model for context generation
    ...
    # Add safety settings for content generation
    safety_settings = [
        {
            "category": "HARM_CATEGORY_HARASSMENT",
            "threshold": "BLOCK_MEDIUM_AND_ABOVE"
    ...
    response = await context_model.generate_content_async(
        context_prompt,
        generation_config=generation_config,
...
"""
    ...
    # Use Gemini for feedback generation with system instruction
    ...
    response = await feedback_model.generate_content_async(
        feedback_prompt,
        generation_config=generation_config
    )
...
async def health_check():
    ...
    try:
        ...
        is_healthy = all([db_healthy, redis_healthy, gemini_healthy])
