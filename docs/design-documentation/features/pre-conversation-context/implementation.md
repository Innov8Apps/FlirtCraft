...
dependencies:
  ...
  - Google Gemini API for context generation
status: approved
---
...
**AI Integration:**
- **Primary**: Google Gemini 2.5 Flash-Lite for context generation
...
### Google Gemini Integration for Context Generation
...
class AIContextService {
  ...
  async generateContext(params: ScenarioParams): Promise<ScenarioContext> {
    ...
    try {
      ...
      let result;
      try {
        result = await this.primaryModel.generateContent(prompt);
      } catch (primaryError) {
      ...
      const rawContext = JSON.parse(response.text());
